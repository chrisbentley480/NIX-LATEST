...
function login(){
	...
	username_sha256b64=sha256_64(strip_string(username));
...
function generatePadlock(){
	...
	setTimeout(function(){
		...
		var padlock_cycles=500000000;
		if (debug){
			padlock_cycles=10;
		}
		
		for (var i=0;i<padlock_cycles;i++){
			padlockHasher();
		...
		var prng_cycles=1000000;
		if (debug){
			prng_cycles=10;
		}
		
		//Cycle the PRNG a million cycles
		for (var i=0;i<prng_cycles;i++){
			padlockPRNG();
		}
...
function openInterface(){
	...
	setTimeout(function(){
		...
		loadConnections();
		setInterval(function () {loadConnections()}, 5000);
	}, 1000);
}
